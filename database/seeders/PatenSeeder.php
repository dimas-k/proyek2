<?php

namespace Database\Seeders;

use Illuminate\Database\Console\Seeds\WithoutModelEvents;
use Illuminate\Database\Seeder;
use Illuminate\Support\Facades\DB;

class PatenSeeder extends Seeder
{
    /**
     * Run the database seeds.
     */
    public function run(): void
    {
        DB::table('paten')->insert([
            [
                'user_id' => 3,
                'nama_lengkap' => "Suhendi",
                'alamat' => "Suhendi",
                'no_telepon' => "081214627146",
                'tanggal_lahir' => "1985-06-25",
                'ktp_inventor' => "Suhendi.pdf",
                'email' => "suhendi@gmail.com",
                'kewarganegaraan' => "Indonesia",
                'kode_pos' => "45273",
                'institusi' => "",
                'jenis_paten' => "Paten sederhana",
                'judul_paten' => "Alat Pemotong rumput otomatis berbasis metafisika melalui do'a",
                'abstrak_paten' => "Suhendi.pdf",
                'deskripsi_paten' => "Suhendi.pdf",
                'pengalihan_hak' => "Suhendi.pdf",
                'klaim' => "Suhendi.pdf",
                'pernyataan_kepemilikan' => "2024-07-01",
                'surat_kuasa' => "Keterangan belum lengkap",
                'gambar_paten' => "Suhendi.pdf",
                'gambar_tampilan' => "Suhendi.pdf",
                'tanggal_permohonan' => "2025-02-02",
                'sertifikat_paten' => "Suhendi.pdf",
                'status' => "Diberi"
            ],
            [
                'user_id' => 3,
                'nama_lengkap' => "Suhendi",
                'alamat' => "Suhendi",
                'no_telepon' => "081214627146",
                'tanggal_lahir' => "1985-06-25",
                'ktp_inventor' => "Suhendi.pdf",
                'email' => "suhendi@gmail.com",
                'kewarganegaraan' => "Indonesia",
                'kode_pos' => "45273",
                'institusi' => "",
                'jenis_paten' => "Paten sederhana",
                'judul_paten' => "Alat Pemotong rumput otomatis berbasis metafisika melalui do'a",
                'abstrak_paten' => "Suhendi.pdf",
                'deskripsi_paten' => "Suhendi.pdf",
                'pengalihan_hak' => "Suhendi.pdf",
                'klaim' => "Suhendi.pdf",
                'pernyataan_kepemilikan' => "2025-02-02",
                'surat_kuasa' => "Keterangan belum lengkap",
                'gambar_paten' => "Suhendi.pdf",
                'gambar_tampilan' => "Suhendi.pdf",
                'tanggal_permohonan' => "2025-02-02",
                'sertifikat_paten' => "Suhendi.pdf",
                'status' => "Diberi"
            ],
            [
                'user_id' => 3,
                'nama_lengkap' => "Suhendi",
                'alamat' => "Suhendi",
                'no_telepon' => "081214627146",
                'tanggal_lahir' => "1985-06-25",
                'ktp_inventor' => "Suhendi.pdf",
                'email' => "suhendi@gmail.com",
                'kewarganegaraan' => "Indonesia",
                'kode_pos' => "45273",
                'institusi' => "",
                'jenis_paten' => "Paten sederhana",
                'judul_paten' => "Alat Pemotong rumput otomatis berbasis metafisika melalui do'a",
                'abstrak_paten' => "Suhendi.pdf",
                'deskripsi_paten' => "Suhendi.pdf",
                'pengalihan_hak' => "Suhendi.pdf",
                'klaim' => "Suhendi.pdf",
                'pernyataan_kepemilikan' => "2025-02-02",
                'surat_kuasa' => "Keterangan belum lengkap",
                'gambar_paten' => "Suhendi.pdf",
                'gambar_tampilan' => "Suhendi.pdf",
                'tanggal_permohonan' => "2025-02-02",
                'sertifikat_paten' => "Suhendi.pdf",
                'status' => "Diberi"
            ],
            [
                'user_id' => 3,
                'nama_lengkap' => "Suhendi",
                'alamat' => "Suhendi",
                'no_telepon' => "081214627146",
                'tanggal_lahir' => "1985-06-25",
                'ktp_inventor' => "Suhendi.pdf",
                'email' => "suhendi@gmail.com",
                'kewarganegaraan' => "Indonesia",
                'kode_pos' => "45273",
                'institusi' => "",
                'jenis_paten' => "Paten sederhana",
                'judul_paten' => "Alat Pemotong rumput otomatis berbasis metafisika melalui do'a",
                'abstrak_paten' => "Suhendi.pdf",
                'deskripsi_paten' => "Suhendi.pdf",
                'pengalihan_hak' => "Suhendi.pdf",
                'klaim' => "Suhendi.pdf",
                'pernyataan_kepemilikan' => "2025-02-02",
                'surat_kuasa' => "Keterangan belum lengkap",
                'gambar_paten' => "Suhendi.pdf",
                'gambar_tampilan' => "Suhendi.pdf",
                'tanggal_permohonan' => "2025-02-02",
                'sertifikat_paten' => "Suhendi.pdf",
                'status' => "Diberi"
            ],

        ]);
}
}

